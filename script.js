...
(function loader() {
  ...
  const tick = (now) => {
    ...
    let v = Math.min(100, easedProgress * 100);
    if (progress >= 1) v = 100;
...
(function manifestoScrollScrub() {
  ...
  if (!strikeLine || !strikeWord || !strikeEl || !repSvg || !repLetters.length)
    return;
  ...
  function setupRepLetters() {
    ...
    const lineHeight =
      parseFloat(getComputedStyle(strikeEl).lineHeight) || fontSize;
    const repFont =
      "italic 400 " + strikeStyle.fontSize + ' "Instrument Serif", serif';
  ...
  if (document.fonts && document.fonts.ready) {
    const fontTimeout = new Promise(function (resolve) {
...
(function voiceFingerprintCanvas() {
  ...
  function draw() {
    ...
    drawRidge(W * 0.35, H * 0.3, 0.05);
    drawRidge(W * 0.28, H * 0.22, 0.04);
    drawRidge(W * 0.2, H * 0.15, 0.03);

    // Wave tracks
    drawWave(H * 0.18, H * 0.07, 0.022, 0.025, 0, "rgba(26,106,255,0.35)", 1.2);
    drawWave(H * 0.32, H * 0.1, 0.018, 0.02, 1.5, "rgba(110,60,255,0.40)", 1.4);
    ...
    drawWave(
      ...
      0.015,
      4.5,
      "rgba(255,138,61,0.40)",
      1.4,
    );
    drawWave(H * 0.86, H * 0.07, 0.019, 0.02, 6, "rgba(255,94,0,0.30)", 1.2);
...
(function typographicStorm() {
  ...
  function noiseFn(x, y) {
    return (
      Math.sin(x * 0.05) * Math.cos(y * 0.05) + Math.sin(x * 0.02 + y * 0.03)
    );
  }
  ...
  function init() {
    ...
    let signalCount = 0;
    const WORD_COUNT = () =>
      Math.floor(Math.min(PERF.stormWords, (W * H) / 5000));
    const MAX_SIGNAL = 24;
  ...
  const phaseAssemble = (p) => Math.max(0, Math.min(1, (p - 0.35) / 0.2));
  const phaseOutput = (p) => Math.max(0, Math.min(1, (p - 0.55) / 0.2));
  ...
  function draw(now) {
    ...
    smoothProgress +=
      (rawProgress - smoothProgress) * clamp(1 - Math.pow(0.9, dt * 60), 0, 1);
    ...
    const pOutput = Math.max(0, Math.min(1, (p - 0.55) / 0.2));
    const pNetwork = Math.max(0, Math.min(1, (p - 0.75) / 0.2));
      ...
      for (let k = 0; k < gridTitles.length; k++) {
        ...
        if (pNetwork > 0) {
          for (let ci = 0; ci < t.connections.length; ci++) {
            ...
            ctx.strokeStyle =
              "rgba(255,94,0," +
              (pNetwork * gOp * 0.18 * (t.alpha + tj.alpha)) / 2 +
              ")";
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
    ...
    if (pa > 0.5) {
      ...
      for (let c = 0; c < totalCols; c++) {
        ...
        ctx.strokeStyle =
          "rgba(26,106,255," + (pa - 0.5) * 2 * 0.4 * lineAlpha + ")";
        ctx.lineWidth = 1;
        ctx.stroke();
      }
    }
  ...
  if (typeof gsap !== "undefined" && typeof ScrollTrigger !== "undefined") {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        const revealed = self.progress > 0.8;
    ...
    overlays.forEach((el, i) => {
      const ranges = [0.02, 0.14, 0.32, 0.5, 0.68, 0.82];
  ...
  function updateFallbackUI(sp) {
    ...
    if (scrollIndicator)
      scrollIndicator.classList.toggle("is-hidden", sp > 0.03);
